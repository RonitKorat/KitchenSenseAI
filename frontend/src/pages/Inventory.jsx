...
const Inventory = () => {
  ...
  return (
    <>
      <div className="text-center bg-gradient-to-br from-green-50 to-green-100 dark:from-gray-900 dark:to-gray-800 transition-colors duration-300">
        <motion.div
        ...
        >
          ...
          {images.length === 0 && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center text-gray-500 dark:text-gray-400 mt-8 w-250 h-50 mx-auto"
            >
          ...
          {images.length > 0 && (
            <div className="mt-10">
              <div className="ml-30 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  gap-6 mt-5 mr-30">
                {images.map((image, index) => (
                  <div key={index}>
                    <img
                      src={image.preview}
                      alt={`Uploaded image ${index + 1}`}
                      className="w-full h-50 object-cover border rounded-2xl "
                    />
                  </div>
                ))}
              </div>
